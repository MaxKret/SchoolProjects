...
def groupSum(start, nums, target):
    '''
    Input: start = idx, nums = [], target = target sum N
    Algorithm:
        ex: start = 0, nums = [2, 4, 8], target = 10
        0:  check if start is >= the length of nums       (check if nums[start] is correct)
        1:  recursively check if groupSum(start+1, nums, target - nums[start]) is True ^^ ; we check if target-nums[s] == 0
        2:  recursively check if groupSum(start+1, nums, target) is True ; the case that nums[s] isn't chosen
        3:  else is False
    '''
...
def groupSum6(start, nums, target):
    ...
    Input: start = idx, nums = [], target = target sum N
    Algorithm:
        0:  check if start is >= the length of nums
        1:  check if nums[s] is 6, meaning its chosen > recursively check target - nums[s]
        2:  recursively check target - nums[s] ; we check if target-nums[s] == 0
        3:  recursively go to start+1 to continue through array; the case that nums[s] isn't chosen
        4:  else is False
    '''
    if start >= len(nums):
    	return target == 0
...
def groupNoAdj(start, nums, target):
    ...
    Algorithm:
        0:  check if start is >= the length of nums
        1:  recursively go to start+1 to continue through array; the case that nums[s] isn't chosen
        2:  recursively check target - nums[s] in a non-adj element; we check if target-nums[s] == 0
        3:  else is False
    '''
    if start >= len(nums):
    	return target == 0
...
def splitArrayHelper(nums, idx, sum1, sum2):
    if idx >= len(nums):
    	return sum1 == sum2
    currVal = nums[idx]
    return splitArrayHelper(nums, idx+1, sum1+currVal, sum2) or splitArrayHelper(nums, idx+1, sum1, sum2+currVal)
...
def splitOdd10Helper(nums, idx, sum1, sum2):
    if idx >= len(nums):
    	return (sum1 % 10 == 0 and sum2 % 2 != 0) or (sum1 % 2 != 0 and sum2 % 10 == 0)
    currVal = nums[idx]
    return splitOdd10Helper(nums, idx+1, sum1+currVal, sum2) or splitOdd10Helper(nums, idx+1, sum1, sum2+currVal)
...
def split53Helper(nums, idx, sum1, sum2):
    if idx >= len(nums):
    	return sum1 == sum2
    currVal = nums[idx]

    if (currVal % 5 == 0):
        return split53Helper(nums, idx+1, sum1+currVal, sum2)
    elif (currVal % 3 == 0):
        return split53Helper(nums, idx+1, sum1, sum2+currVal)
    else:
        return split53Helper(nums, idx+1, sum1+currVal, sum2) or split53Helper(nums, idx+1, sum1, sum2+currVal)
